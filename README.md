...
### ダッシュボード画面
-  総マイル、ランク、今月の活動表示
-  未達成ミッションの中で報酬マイルが最大のものを「おすすめミッション」として表示
-  ランク判定機能（マイル数に応じて自動判定、次ランクまでの進捗バー表示）
-  週間活動グラフ(Chart.js を使用)
-  過去7日間の活動件数を棒グラフで表示
...
-  月別マイル獲得グラフ・累積マイル推移グラフ・カテゴリ別マイル獲得グラフ
-  半期ごとにどのような活動を行ったかを確認可能
...
### 管理者画面
コマンドパレットを実装（Ctrl + K で起動"admin" 入力で管理者ページへ遷移）
-  ユーザー管理
-  ミッション管理
-  報酬決定
-  報酬配布管理
-  半期目標一括アップロード
-  AI自動抽出アップロード
-  半期設定（半期の開始日・終了日の設定、自動リセット ON / OFF、手動リセット、半期の削除）
-  管理者設定

### Slack連携
-  Slackログイン / Slackユーザー紐付け（`slack_id`）
-  Slash Command `/mission` でミッション完了機能（実装中・不具合対応中）
-  リマインド機能（週次進捗レポート・期限リマインド・ランクアップ通知）
