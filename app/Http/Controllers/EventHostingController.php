...
class EventHostingController extends Controller
{
    /**
     * イベント企画・開催フォーム表示
     */
    ...
    {
        // event_organizer ミッションを取得（存在する場合）
        $mission = Mission::where('key', 'event_organizer')->first();
        
        return view('events.hosting.create', compact('mission'));
    }
    ...
    public function store(
        ...
        return redirect()
            ->route('timeline.index', ['type' => 'event_hosting'])
            ->with('success', 'イベント企画・開催情報を登録しました');
    }
}
