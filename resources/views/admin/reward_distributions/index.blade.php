...
<div class="max-w-6xl mx-auto px-4 py-8 space-y-8">
    ...
    {{-- 新規追加 --}}
    <form method="POST"
          action="{{ route('admin.reward-distributions.store') }}"
    ...
    <div class="bg-white rounded-xl shadow divide-y">
        @foreach($distributions as $d)
            <div
                ...
                {{ $d->is_expired ? 'opacity-50 bg-gray-50' : '' }}"
            >
                {{-- 左側 --}}
                <div>
                    <p class="font-semibold flex items-center gap-2">
                        ...
                        @if($d->is_expired)
                            <span class="text-xs px-2 py-0.5 rounded-full bg-gray-400 text-white">
                                期限切れ
                            </span>
                        @endif
                    </p>
